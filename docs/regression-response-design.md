...
- A node assigned deployment X that reports the predecessor's `archive_sha256` with
  `settled=true` after its report previously showed X (or showed X unsettled) has attempted X
  and rolled back. This is the report sequence the supervisor already publishes: the heartbeat
  emits every tick, `settled=false` during the confirmation window, and the committed digest
...
The e2e chaos scenario gains a poisoned-release case asserting the halt lands before the
second cohort is admitted.
