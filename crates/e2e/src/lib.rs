...
//! (Ctx, Proc, Service, publish, serve, waits); `fixtures` holds the direct `Node` agent config
...
//! standalone kill fuzzer in `crates/killfuzz`) depend on this library, so the fragile TUF/agent
