...
/// from the `agent` TUF product while the release's own hook manages the workload.
...
fn desired_agent(dir: &Path) -> R<PathBuf> {
    let pointer = dir.join("launcher-state/desired-agent");
    let text = std::fs::read_to_string(&pointer).map_err(str_err)?;
    let mut lines = text.lines();
    if lines.next() != Some("agent-v1") {
        return fail(format!("invalid desired-agent header in {pointer:?}"));
    }
    let path = lines
        .next()
        .filter(|line| !line.is_empty())
        .ok_or_else(|| format!("missing path in desired-agent: {text:?}"))?;
    if lines.next().is_some() {
        return fail(format!("trailing data in desired-agent: {text:?}"));
    }
...
pub(crate) fn agent_self_update(ctx: &Ctx) -> R {
    ...
    // ready, and the launcher commits the desired-agent pointer.
    ctx.publish(&dir, "agent", "2.0.0", &agent_v(ctx, "2.0.0"))?;
    ...
    let expected_v2_sha = sha256_hex(&agent_v(ctx, "2.0.0"))?;
    if !desired.is_file() || sha256_hex(&desired)? != expected_v2_sha {
        return fail(format!(
            "desired-agent did not advance to the staged v2 binary: {desired:?}"
        ));
    }
    ok(&format!(
...
pub(crate) fn agent_self_update_rollback(ctx: &Ctx) -> R {
    ...
    ctx.publish(&dir, "agent", "2.0.0", &broken)?;

    let rejected = boot.wait_for_log("rejecting", EVENT_TIMEOUT);
    let recorded = boot.wait_for_log("recorded rejected agent candidate", EVENT_TIMEOUT);
    ...
    let records = boot.log_count("recorded rejected agent candidate");
    std::thread::sleep(Duration::from_secs(6));
    let retries = boot.log_count("rejecting") - rejections;
    let re_records = boot.log_count("recorded rejected agent candidate") - records;
    ...
    if !desired.is_file() || sha256_hex(&desired)? != sha256_hex(&agent_v1)? {
        return fail(format!(
            "desired-agent did not remain on the committed v1 binary: {desired:?}"
        ));
    }
...
pub(crate) fn agent_post_ready_crash_rolls_back(ctx: &Ctx) -> R {
    ...
    let unstable = ctx
        .work
        .join(format!("build/agent-post-ready-crash-2.0.0{}", ctx.exe));
    ...
    ctx.publish(&dir, "agent", "2.0.0", &unstable)?;
    ...
    let predecessor_returned =
        boot.wait_for_log("recorded rejected agent candidate", EVENT_TIMEOUT);
