...
fn launcher() -> Command {
    Command::new(env!("CARGO_BIN_EXE_updated-launcher"))
}
...
#[cfg(unix)]
fn start_launcher(tag: &str, agent: &std::path::Path) -> (tempfile::TempDir, std::process::Child) {
    let (guard, state) = temp_dir(tag);
    let child = launcher()
        .args(["--state-dir", state.to_str().unwrap()])
        .args(["--config", "/unused/config.toml"])
        .args(["--agent", agent.to_str().unwrap()])
        .spawn()
        .unwrap();
    (guard, child)
}
...
fn missing_path_value_is_a_usage_error() {
    let output = launcher().arg("--state-dir").output().unwrap();
    assert_eq!(output.status.code(), Some(2));
    let stderr = String::from_utf8(output.stderr).unwrap();
    assert!(stderr.contains("--state-dir needs a path"), "{stderr}");
    assert!(stderr.contains("usage: updated-launcher"), "{stderr}");
}

#[test]
fn first_boot_without_a_agent_fails_closed() {
    ...
    let stderr = String::from_utf8(output.stderr).unwrap();
    assert!(
        stderr.contains("fatal: no committed agent and no --agent to seed one"),
        "{stderr}"
    );
}
...
fn the_config_path_defaults_so_a_standard_deployment_never_names_it() {
    // Omitting --config must NOT be a usage error: a node's config has one canonical
    // location (`control::DEFAULT_CONFIG`) and every deployment adapter relies on that
    // default rather than restating a path. Reaching the *agent* fail-closed check (exit 1)
...
fn help_prints_the_complete_operator_contract() {
    let output = launcher().arg("--help").output().unwrap();
    assert!(output.status.success());
    let stderr = String::from_utf8(output.stderr).unwrap();
    for required in [
        "usage: updated-launcher",
        "--state-dir",
        "--config",
        "--agent",
        "--ready-timeout",
...
fn the_launcher_stays_alive_until_sigterm_then_exits_cleanly() {
    ...
    let (_script_tmp, agent) = agent_script(
        "steady-agent",
        &format!(
    ...
    let (_state_tmp, mut child) = start_launcher("steady-state", &agent);
    assert!(
...
fn start_launcher_backoff_probe(
    ...
    let mut child = launcher()
        ...
        .env("UPDATED_LAUNCHER_BACKOFF_BASE_MS", "600000")
...
fn agent_backoff_is_interrupted_by_shutdown() {
    ...
    let (_script_tmp, agent) = agent_script("failed-agent", "exit 7");
    let (_state_tmp, mut child, log) = start_launcher_backoff_probe("backoff", &agent);
