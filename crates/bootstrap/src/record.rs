...
//! which agent binary is committed (`desired-supervisor`) — forward on a successful handoff
//! and leaves it put on a failed one (that is the rollback). And it drops one dumb marker
//! for the agent to interpret on recovery: `rejected-supervisor`, the path of a candidate
