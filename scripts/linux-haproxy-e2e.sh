...
case "$(uname -m)" in
  ...
  *) echo "unsupported HAProxy E2E architecture: $(uname -m)" >&2; exit 1 ;;
esac
...
cleanup() {
  ...
  [[ -n "$STACK_PID" ]] && pkill -TERM -P "$STACK_PID" 2>/dev/null
...
publish() {
  ...
  "$BIN/server" publish-app --repo "$RELEASE_REPO" --keys "$RELEASE_KEYS" --product app \
    --channel stable --version "$version" --bundle "$PLATFORM=$tree"
  assign "$version"
}
...
rm -rf "$WORK"
mkdir -p "$BIN" "$WORK/agent-state"
if [[ -z "${UPDATED_E2E_BIN_DIR:-}" ]]; then
  (cd "$ROOT" && cargo build --release -p server -p agent)
fi
# CI may reuse the exact binaries built into its Linux E2E image.
cp "${UPDATED_E2E_BIN_DIR:-$ROOT/target/release}/"{server,updated-agent} "$BIN/"
# The reconciler and its helpers are the real, tested bundle scripts under scripts/haproxy/ —
...
# `converge` runs after projecting a release when no master is up. Every subsequent upgrade is
...
for tree in "$WORK"/bundle-*; do
  ...
  cp "$BIN/lifecycle" "$BIN/lib.sh" "$tree/bin/"
  printf '%s\n' '{"schema":1,"deploy":{"argv":["./bin/lifecycle"],"timeoutSeconds":10},"inspect":{"argv":["./bin/lifecycle"],"timeoutSeconds":10},"replay":{"policy":"safe"},"recovery":{"policy":"command","command":{"argv":["./bin/lifecycle"],"timeoutSeconds":10},"replay":{"policy":"safe"}},"health":{"argv":["./bin/lifecycle"],"timeoutSeconds":10}}' >"$tree/.updated-execution.json"
done

cat >"$RUNTIME" <<EOF
{"product":"app","channel":"stable","installRoot":"$INSTALL","repository":{"metadataLimit":1048576,"targetLimit":536870912,"transportTimeoutSeconds":5},"storage":{"inactiveReleases":2,"inactiveBytes":1073741824,"inactiveRepositoryCaches":2},"timeouts":{"checkIntervalSeconds":1,"healthGraceSeconds":4,"healthSuccesses":1,"healthIntervalSeconds":1,"refreshRetrySeconds":1,"confirmationWindowSeconds":2}}
EOF
...
"$BIN/server" export-enrollment --repo "$ROUTING_REPO" --assignment assignments/agents/agent.json \
  ...
  --output "$WORK/agent-state/enrollment.json"
...
cp "$CERTS/client.crt" "$WORK/agent-state/agent.crt"
cp "$CERTS/client.key" "$WORK/agent-state/agent.key"
...
# no workload process at all. The serving HAProxy above is the proof that `converge` did the starting,
...
# Preserve every request across reloads, with a five-second response budget below HAProxy's
# ten-second client timeout. Virtualized CI can delay first byte beyond one second even in a
# standalone HAProxy reload with no updater. Keep exact timing diagnostics for real failures.
( while true; do
    body="$(curl -fsS --max-time 5 --write-out '%{stderr} connect=%{time_connect} first-byte=%{time_starttransfer} total=%{time_total}' "http://127.0.0.1:$HTTP_PORT/" 2>"$WORK/probe-error")" || {
      printf '%s unavailable: %s\n' "$(date -u +%T)" "$(cat "$WORK/probe-error")" >>"$TRAFFIC_LOG"
      continue
    }
...
for _ in 1 2; do
  env UPDATED_OPERATION=converge UPDATED_ATTEMPT_ID=haproxy-idempotency-replay UPDATED_INSTALL_ROOT="$INSTALL" UPDATED_STATE_DIR="$INSTALL/execution/state/haproxy" UPDATED_RESULT_FILE="$WORK/replay-result.json" UPDATED_PAYLOAD_ROOT="$release2" UPDATED_PAYLOAD_VERSION=2.0.0 "$BIN/lifecycle"
done
...
[[ "$(cat "$INSTALL/runtime/haproxy.pid")" == "$master_pid" ]] || fail "replayed converge restarted the master"
[[ "$(pgrep -P "$master_pid" | sort -n | tail -n1)" == "$replay_worker" ]] || fail "replayed converge needlessly replaced the live worker"
[[ "$(stat -Lc '%d:%i' "/proc/$master_pid/exe")" == "$replay_inode" ]] || fail "replayed converge needlessly re-execed the live binary"
...
grep -q 'payload HAProxy configuration failed validation' "$STACK_LOG" || fail "invalid binary validation failure was not recorded"
...
env UPDATED_OPERATION=converge UPDATED_ATTEMPT_ID=haproxy-start-duty UPDATED_INSTALL_ROOT="$INSTALL" UPDATED_STATE_DIR="$INSTALL/execution/state/haproxy" UPDATED_RESULT_FILE="$WORK/start-duty-result.json" UPDATED_PAYLOAD_ROOT="$release4" UPDATED_PAYLOAD_VERSION=4.0.0 "$BIN/lifecycle"
restarted_pid="$(cat "$INSTALL/runtime/haproxy.pid")"
[[ "$restarted_pid" != "$master_pid" ]] || fail "converge reported a master that is the one we killed"
kill -0 "$restarted_pid" 2>/dev/null || fail "converge returned success without a running master"
...
env UPDATED_OPERATION=converge UPDATED_ATTEMPT_ID=haproxy-recycled-pid UPDATED_INSTALL_ROOT="$INSTALL" UPDATED_STATE_DIR="$INSTALL/execution/state/haproxy" UPDATED_RESULT_FILE="$WORK/recycled-pid-result.json" UPDATED_PAYLOAD_ROOT="$release4" UPDATED_PAYLOAD_VERSION=4.0.0 "$BIN/lifecycle"
kill -0 "$foreign" 2>/dev/null || fail "converge killed the process named by a recycled pid file"
[[ ! -s "$FOREIGN_SIGNALS" ]] || fail "converge signalled the process named by a recycled pid file"
adopted="$(cat "$INSTALL/runtime/haproxy.pid")"
[[ "$adopted" != "$foreign" ]] || fail "converge left an unrelated process standing in for the master"
kill -0 "$adopted" 2>/dev/null || fail "converge did not start a master beside the unrelated process"
